...
void StudentManager::addStudent(Database &db)
{
    try
    {
        ...
        if (isEmpty(name) || isEmpty(surname) || isEmpty(department) || isEmpty(email))
        {
...
void StudentManager::listStudents(Database &db)
{
    try
    {
        pqxx::work txn(db.getConnection());
        pqxx::result r =
            txn.exec("SELECT id, name, surname, department, email FROM students ORDER BY id");
...
void StudentManager::updateStudent(Database &db)
{
    try
    {
        ...
        pqxx::result r = txn.exec_params(
            "UPDATE students SET name=$1, surname=$2, department=$3, email=$4 WHERE id=$5", name,
            surname, department, email, id);
